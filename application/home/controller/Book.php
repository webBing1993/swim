...
class Book extends Base
{
    ...
    public function search(){
        ...
        foreach($list as $key => $value){
            if (empty($value['header'])){  //  获取头像
                ...
                $list[$key]['header'] = param_to($value['avatar'], \think\Config::get('de_header'));
            }else{
                $list[$key]['header'] = get_cover($value['header'], 'path');
            }
    ...
    public function  userinfo(){
        ...
        if (empty($userinfo['header'])){
            ...
            $userinfo['header'] = param_to($userinfo['avatar'], \think\Config::get('de_header'));
        }else{
            $userinfo['header'] = get_cover($userinfo['header'], 'path');
        }
